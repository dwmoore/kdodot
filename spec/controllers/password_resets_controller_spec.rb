...
describe PasswordResetsController do
	...
	describe "POST create" do
		context "with a vaild user and email" do
		...
		context "with no user found" do
			it "renders the new page" do
				post :create, email: 'none@email.com'
				expect(response).to render_template('new')
			end
	...
	describe "PATCH update" do
		context "with no token found" do
			it "renders the edit page" do
				patch :update, id: 'notfound', user: { password: "password1", password_confirmation: "password1" }
				expect(response).to render_template('edit')
			end
